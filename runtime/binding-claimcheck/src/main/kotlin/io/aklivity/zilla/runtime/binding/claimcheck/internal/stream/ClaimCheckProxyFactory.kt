    ...
    private companion object {
        ...
        val SUPPORTED_HTTP_METHOD: Predicate<HttpHeaderFW> = Predicate { header ->
            header.name().equals(HEADER_METHOD_NAME) &&
                    (header.value().asString() in listOf("POST", "PUT", "GET"))
        }
    ...
    private inner class HttpProxy(
        ...
        private fun onHttpBegin(begin: BeginFW) {
            ...
            try {
                ...
                println("HttpProxy: BEGIN seq=$sequence ack=$acknowledge traceId=$traceId authorization=$authorization")

                val httpBeginEx = extension.get(httpBeginExRO::tryWrap)
                val method = httpBeginEx?.headers()?.matchFirst { it.name().equals(HEADER_METHOD_NAME) }?.value()?.asString()
                println("HttpProxy: Detected HTTP method=$method")

                if (method == "GET") {
                    val pathHeader = httpBeginEx.headers().matchFirst { h -> h.name().asString() == ":path" }
                    val path = pathHeader?.value()?.asString() ?: run {
                        println("HttpProxy: Missing :path header — sending 400")
                        doHttpReset(traceId, HEADER_STATUS_VALUE_400)
                        return
                    }

                    val parts = path.split("/")
                    val objectKey = parts.lastOrNull() ?: run {
                        println("HttpProxy: Path missing objectKey — sending 400")
                        doHttpReset(traceId, HEADER_STATUS_VALUE_400)
                        return
                    }

                    println("HttpProxy: Handling GET -> path=$path, objectKey=$objectKey")
                    handleHttpGet(traceId, authorization, objectKey)
                    return
                }

                println("HttpProxy: Non-GET method detected, entering PUT/upload flow")
                initialSeq = sequence
        ...
        private fun handleHttpGet(traceId: Long, authorization: Long, objectKey: String) {
            println("HttpProxy: >>> Fetching object from MinIO: objectKey=$objectKey")
            try {
                ...
                println("HttpProxy: Retrieved ${bytes.size} bytes from MinIO for key=$objectKey")

                val payloadBuffer = UnsafeBuffer(bytes)
                val payload = OctetsFW().wrap(payloadBuffer, 0, bytes.size)
                ...
                val httpBeginEx = httpBeginExRW.wrap(extBuffer, 0, extBuffer.capacity())
                    ...
                    .headersItem { h -> h.name(HEADER_CONTENT_LENGTH_NAME).value(String16FW(bytes.size.toString())) }
                    .headersItem { h -> h.name(HEADER_CONTENT_TYPE_NAME).value(String16FW("application/octet-stream")) }
                    .build()

                doHttpBegin(traceId, authorization, 0L, httpBeginEx)
                doHttpData(traceId, authorization, 0L, bytes.size, 0, payload)
                doHttpEnd(traceId, authorization)

                println("HttpProxy: <<< Sent HTTP 200 with ${bytes.size} bytes for key=$objectKey")

            } catch (e: Exception) {
                println("HttpProxy: !!! Failed to fetch object $objectKey: ${e.message}")
                e.printStackTrace()
            ...
            try {
                ...
                try {
                    ...
                    val presignedUrl = try {
                        ...
                        println("HttpProxy: Could not generate presigned URL: ${e.message}")
                        null
                    }

                    println("HttpProxy: Generated presigned URL=$presignedUrl")
                    ...
                    val httpBeginEx = httpBeginExRW.wrap(extBuffer, 0, extBuffer.capacity())
                        ...
                        .apply {
                            // Always include both UUID and URL (if available)
                            headersItem { h -> h.name(String8FW("X-Claim-UUID")).value(String16FW(claimKey)) }
                            if (presignedUrl != null) {
                                headersItem { h -> h.name(String8FW("X-Claim-URL")).value(String16FW(presignedUrl)) }
                            }

                            // Retain any other configured headers
                            resolved.headers.forEach { (name, value) ->
                                val replaced = value
                                    .replace("uuid", claimKey)
                                    .replace("url", presignedUrl ?: claimKey)
                    ...
                    // Send HTTP Begin + End
                    println("HttpProxy: Sending HTTP Begin + End with traceId=$traceId")
                    doHttpBegin(traceId, authorization, 0L, httpBeginEx)
                    doHttpEnd(traceId, authorization)

                } catch (ex: Exception) {
                    println("HttpProxy: Error during upload or response: ${ex.message}")
                    ex.printStackTrace()
                    doHttpReset(traceId, HEADER_STATUS_VALUE_500)
                } finally {
