...
class DefaultImageLinker(
    ...
    fun link(jar: Path, targetDir: Path): Either<ImageLinkError, Path> {
        ...
        if (dryRun) {
            feedback("🧪 [dry-run] Would run jlink with JAR: $jar to $imagePath")
        ...
        try {
            Files.createDirectories(imagePath)
            feedback("🔗 Running jlink with JAR: $jar to $imagePath")
...
data class ImageLinkError(override val message: String): Exception()